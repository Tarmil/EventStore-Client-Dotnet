	...
	public class create_on_all_stream
		...
		public Task the_completion_succeeds()
			=> _fixture.Client.CreateAsync(
				SystemStreams.AllStream, "existing", new PersistentSubscriptionSettings(), TestCredentials.Root);

		[Fact]
		public Task throws_argument_exception_if_wrong_start_from_type_passed()
			=> Assert.ThrowsAsync<ArgumentException>(() => _fixture.Client.CreateAsync(
				SystemStreams.AllStream, "existing",
